...
def P_Net(training=True):
    ...
    if training:
        ...
        face_cls = Conv2D(3, (1, 1), activation='softmax')(x)
        bbox_reg = Conv2D(5, (1, 1), name='bbox_reg')(x)
        ldmk_reg = Conv2D(11, (1, 1), name='ldmk_reg')(x)
...
def P_Net_alter1(training=True):
    ...
    x = Conv2D(16, (3, 3), strides=1, padding='valid', name='conv1')(inp)
    x = PReLU(shared_axes=(1, 2), name='prelu1')(x)
    x = MaxPool2D((2, 2), strides=2)(x)
